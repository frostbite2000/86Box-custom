...
#ifdef ENABLE_IT8712_LOG
int it8712_do_log = ENABLE_IT8712_LOG;
static void
it8712_log(const char *fmt, ...)
{
    va_list ap;
...
#else
#    define it8712_log(fmt, ...)
#endif

static void it8712_reset(void *priv);

static void
it8712_unlock(uint8_t val, it8712_t *dev)
{
    if (val == 0x87) { /* The unlock mechanism of the CUSL2-C doesn't work as intended so we just unlock it with 87h only for now */
        it8712_log("it8712 Unlock: Unlocking\n");
        dev->unlock = 1;
    }
}

static void
it8712_sw_lock(uint8_t val, it8712_t *dev)
{
    if (val & 0x80)
        dev->sw_lock = val;
    else
...
it8712_fdc(it8712_t *dev)
{
    ...
    if (dev->enable[0] & 1) {
        it8712_log("it8712 FDC: Enabled with Base: 0x%x IRQ: %d and DMA: %d\n", base, irq, dma);
...
static void
it8712_uart(int uart, it8712_t *dev)
{
    ...
    if (dev->enable[2 + uart]) {
        it8712_log("it8712 Serial %c: Enabled with Base: 0x%x IRQ: %d\n", 'A' + uart, base, irq);
...
it8712_lpt(it8712_t *dev)
{
    ...
    if (dev->enable[1] & 1) {
        it8712_log("it8712 LPT1: Enabled with Base: 0x%x IRQ: %d\n", base, irq);
        lpt1_setup(base);
        lpt1_irq(irq);
        lpt2_irq(irq);
    }
...
static void
it8712_write(uint16_t addr, uint8_t val, void *priv)
{
    it8712_t *dev = (it8712_t *) priv;

    if (addr == 0x2e) {
        if (dev->unlock)
            dev->index = val;
        else
            it8712_unlock(val, dev);
    } else if (addr == 0x2f) {
        switch (dev->index) {
            /* Global Registers */
            case 0x02: /* Configure Control */
                if (val & 2) {
                    it8712_log("it8712 Unlock: Locking\n");
                    dev->unlock = 0;
                }

                if (val & 1) {
                    it8712_log("it8712: Resetting\n");
                    it8712_reset(dev);
                }
                break;
...
static uint8_t
it8712_read(uint16_t addr, void *priv)
{
    const it8712_t *dev = (it8712_t *) priv;
...
static void *
it8712_init(const device_t *info)
{
    it8712_t *dev = (it8712_t *) malloc(sizeof(it8712_t));
    memset(dev, 0, sizeof(it8712_t));
    ...
    io_sethandler(info->local, 2, it8712_read, NULL, NULL, it8712_write, NULL, NULL, dev); /* Ports 2Eh-2Fh: ITE it8712 */
...
const device_t it8712_device = {
    .name          = "ITE IT8712",
    .internal_name = "it8712",
    .flags         = 0,
    .local         = 0x2e,
    .init          = it8712_init,
    .close         = it8712_close,
